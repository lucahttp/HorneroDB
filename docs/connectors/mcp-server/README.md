...
The MCP server should use an API Key with a role that has explicit `mcp` system permissions. This allows the AI to perform configuration and data tasks while being restricted by the same RBAC system as human users.

### Recommended Role JSON
```json
{
  "__system__": {
    "mcp": "all",
...
```
*Note: Granting `mcp: all` gives the AI assistant full access to respond to natural language queries about the system schema.*
